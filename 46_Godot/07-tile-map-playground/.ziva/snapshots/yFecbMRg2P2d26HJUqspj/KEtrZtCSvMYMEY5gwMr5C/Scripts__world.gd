...
func _clear_selection() -> void:
	...
	arrow_overlay.clear_path()
	arrow_overlay.hide_facing_cursor()
	# Reset label to actual available tiles
	player._update_tiles_label()
...
func _input(event: InputEvent) -> void:
	...
	if event.button_index == MOUSE_BUTTON_LEFT:
		...
		# Update tiles label to show remaining after selection
		var remaining_after_selection: int = player.available_tiles - _selected_tiles
		if player.tiles_label:
			player.tiles_label.text = str(remaining_after_selection) + "/" + str(player.MAX_AVAILABLE_TILES)
...
func _on_next_turn_button_pressed() -> void:
	...
	# Deduct tiles for movement
	player.available_tiles -= _selected_tiles
	player._update_tiles_label()
	
	# Deduct tiles for facing change if applicable
	if _selected_facing != "" and _selected_facing != _selected_direction:
		var facing_cost: int = player.facing_cost(_selected_direction, _selected_facing)
		player.available_tiles -= facing_cost
		player._update_tiles_label()
