...
@onready var tiles_label: Label = $TilesUI/TilesLabel
...
func set_facing(direction: String) -> void:
	if not ISO_DIRECTIONS.has(direction):
		return
	var cost: int = facing_cost(facing_direction, direction)
	if cost > available_tiles:
		return  # Not enough tiles to perform this rotation
	available_tiles -= cost
	_update_tiles_label()
	facing_direction = direction
	var anim_base: String = DIR_ANIMATION.get(direction, "down-right")
	sprite.play(anim_base + "-idle")
...
func _update_tiles_label() -> void:
	if tiles_label:
		tiles_label.text = str(available_tiles) + "/" + str(MAX_AVAILABLE_TILES)
