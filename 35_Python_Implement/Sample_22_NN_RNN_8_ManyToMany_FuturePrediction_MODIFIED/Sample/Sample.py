...
def getTestSets():
    ...
    #fitting shapes to batch size 5, 17 / 4, 15
...
def nextFutureStep(train_data):
    pred, train_data = predict_future(train_data)
    return pred[-1], train_data

def startPredictions(train_data):
    train_data.X_test_splitted[~np.isfinite(train_data.X_test_splitted)] = 0

    future_results = np.array([]) #initialize numpy array
    y_pred, train_data = predict_future(train_data) #create initial result array

    for next in range(future_steps):
        print('future step ' + str(next) + ' of ' + str(future_steps)) #display info
        new_item, train_data = nextFutureStep(train_data) #assign item
