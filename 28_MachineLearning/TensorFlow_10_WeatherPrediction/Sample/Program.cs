...
//https://github.com/SciSharp/SciSharp-Stack-Examples/blob/master/src/TensorFlowNET.Examples/TimeSeries/WeatherPrediction.cs
//looks like missing classes
...
namespace Sample
{
    public class WeatherPrediction : SciSharpExample, IExample
    {
        ...
        public bool Run()
        {
            ...
            task = wizard.AddTimeSeriesTask<string>(new TaskOptions
            {
                WeightsPath = @"timeseries_linear_v1\saved_weights.h5"
            });
            task.SetModelArgs(new
            {
                InputWidth = 3,
                LabelWidth = 1,
    ...
    class Program
    {
        ...
        {
            WeatherPrediction x = new WeatherPrediction();
            x.Run();
        }
    }
}