...
ContractResolver = new CamelCasePropertyNamesContractResolver()
        ...
        private HttpClient GetClient()
        {
            ...
            client.DefaultRequestHeaders.Add("X-Parse-Application-Id", "TWÓJ_KLUCZ_APLIKACJI");
            client.DefaultRequestHeaders.Add("X-Parse-REST-API-Key", "TWÓJ_KLUCZ_API");
...
{ "skip", skip.ToString() }
                ...
                await _opQueue.Enqueue(10, () => client.GetAsync(OrdersUrl + "?" +
                ...
                JsonConvert.DeserializeObject<IEnumerable<Order>>(ordersJson);
                List<Task> tasks = new List<Task>();
                foreach (Order order in orders)
                {
                    tasks.Add(GetItemsByOrderNumberAsync(order.OrderNumber)
...
{ "where", orderNumberJson }
            ...
            string param = await encodedContent.ReadAsStringAsync().ConfigureAwait(false);
            HttpResponseMessage httpResponseMessage =
            await _opQueue.Enqueue(15, () => client.GetAsync(ItemsUrl + "?" +
            param)).ConfigureAwait(false);
            httpResponseMessage.EnsureSuccessStatusCode();
            ...
            JsonConvert.DeserializeObject<IEnumerable<Item>>(ordersJson);
            return items;
        }
    }
    public async Task<Order> InsertOrderAsync(Order order)
    {
        ...
        {
            HttpResponseMessage httpResponseMessage =
            await _opQueue.Enqueue(1, () => client.PostAsync(OrdersUrl,
            new StringContent(
    ...
    public async Task<Item> InsertItemAsync(Item item)
    {
        ...
        {
            HttpResponseMessage httpResponseMessage =
            await _opQueue.Enqueue(3, () => client.PostAsync(ItemsUrl,
            new StringContent(
