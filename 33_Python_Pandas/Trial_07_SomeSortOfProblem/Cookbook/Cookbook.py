...
dfAcc = pd.read_csv(models_accuracies_path, sep='|', dtype = 'float64', converters = {'file_name': str})
modelFilePaths = []
modelFilePaths.append(dfAcc['file_name'].iloc[0])
#dfAcc = dfAcc.query('val_acc >= 0.85')
dfAcc = dfAcc[(dfAcc['val_acc'] >= 0.85) & (dfAcc['acc'] >= 0.85)]
dfAcc = dfAcc[(dfAcc['val_loss'] <= 0.01) & (dfAcc['acc'] <= 0.01)]
